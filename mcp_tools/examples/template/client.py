#!/usr/bin/env python3
"""
Template for an MCP client.
This is a starting point for creating a client for your custom MCP tool.
"""
...
parser = argparse.ArgumentParser(description="Custom Tool Client")
parser.add_argument(
    "--format",
...
async def interactive_loop():
    ...
    output.print("Custom Tool Client", color="green")
    ...
    output.print("  save KEY VALUE - Save data")
    output.print("  get KEY - Get data")
    output.print("  fetch URL - Fetch data from URL")
    output.print("  history [TOOL_NAME] - Get tool history from server")
    output.print("  local_history [TOOL_NAME] - Get local tool history")
    ...
    async with Client(transport) as client:
        ...
        while True:
            try:
                ...
                elif command == "local_history":
                    tool_name = parts[1] if len(parts) > 1 else None
                    output.info("Getting local tool history")
